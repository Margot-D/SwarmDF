...
def build_left_sidebar(gui):
    ...
    gui.header_lsidebar = customtkinter.CTkLabel(gui.left_sidebar, text="SwarmDF \n User interface", font=FONT_TITLE)
    gui.header_lsidebar.grid(row=0, column=0, padx=20, pady=(20, 50))

    # Run Lompe analysis
    gui.checkbox_runlompe = customtkinter.CTkCheckBox(master=gui.left_sidebar, text=f"Run Lompe analysis", font=FONT_NORMAL)
    gui.checkbox_runlompe.grid(row=3, column=0, padx=20, pady=(20,20))
    gui.checkbox_runlompe.select()

    # Run LompeOSSE validation
    gui.checkbox_lompeosse = customtkinter.CTkCheckBox(gui.left_sidebar, text='Run LompeOSSE validation', font=FONT_NORMAL)
    gui.checkbox_lompeosse.grid(row=4, column=0, padx=20, pady=(20,20))

    # Generate Python script
    gui.switch_pythonscript = customtkinter.CTkSwitch(master=gui.left_sidebar, text=f"Generate Python code", font=FONT_NORMAL) 
    ...
    gui.entry_filename = customtkinter.CTkEntry(master=gui.left_sidebar, placeholder_text="MyFile.py", font=FONT_NORMAL)
    ...
    # Demo mode
    gui.switch_demo = customtkinter.CTkSwitch(master=gui.left_sidebar, text=f"Demo mode", command=lambda: apply_example_date(gui), font=FONT_NORMAL) 
    ...
    # Apparence mode
    gui.label_appearance = customtkinter.CTkLabel(gui.left_sidebar, text="Appearance Mode:", anchor="w", font=FONT_NORMAL)
    gui.label_appearance.grid(row=10, column=0, padx=20, pady=(10, 0))
    gui.optmenu_appearance = customtkinter.CTkOptionMenu(gui.left_sidebar, values=["Light", "Dark", "System"], font=FONT_NORMAL, command=change_appearance_mode)
    gui.optmenu_appearance.grid(row=11, column=0, padx=20, pady=(10, 10))
    gui.optmenu_appearance.set("Dark")

    # Scaling option
    gui.label_scaling = customtkinter.CTkLabel(gui.left_sidebar, text="UI Scaling:", anchor="w", font=FONT_NORMAL)
    gui.label_scaling.grid(row=12, column=0, padx=20, pady=(10, 0))
    gui.optmenu_scaling = customtkinter.CTkOptionMenu(gui.left_sidebar,  values=["80%", "90%", "100%", "110%", "120%"], font=FONT_NORMAL, command=change_scaling)
